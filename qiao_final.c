...
void DisposeKey(void) //处理按键事件
{
	switch(cKeyCode)
	{
		...
		*/
		case 0x80:						//P3^5
			blue=0;red=1;				//同上
			setTemp = setTemp+100;  //SetTemp设置的温度++
			nDelayKey = 2000;		
			//displayTemp();
			//delay(10);
			break;
		case 0x40:						//P3^6
			blue=0;red=0;
			setTemp = setTemp-100;  //SetTemp设置温度--
			nDelayKey = 2000;
			//displayTemp();
			//delay(10);
			break;
...
void IntT1(void) interrupt 3
{
	...
	else
	{
		...
		if(nDelayKey == 0)
		{
			cKeyCode = P3 &0xc0;		  // 0111 0000  P3^456  这里你要设置看你的按键具体是多少,然后去判断是否按下了
			if(cKeyCode != cKey)	
			{
				cKeyCode = 0;				
			}
